import React, { Component } from 'react';
import './../styles/Profile.scss';

const Profile = (props) => {
  return (
    <div>
      <img
        src="data:image/png;base64,"
        alt="pic"
      />

      <h2>Name</h2>
      {/* <h1> {props.user.name} </h1> */}
      {/* <img src={this.props.user.image}/> */}
      <p className="p">
        Cupidatat anim esse commodo consectetur laboris ex incididunt Lorem
        eiusmod dolore ipsum quis excepteur esse.Nulla qui elit aute nulla et.
        Ipsum labore incididunt voluptate aliquip commodo aliquip nulla
        consectetur Lorem minim sunt. Duis cillum reprehenderit officia eiusmod
        elit tempor. Aliquip cupidatat est anim ipsum aliqua commodo consectetur
        occaecat.
      </p>
    </div>
  );
};

export default Profile;
